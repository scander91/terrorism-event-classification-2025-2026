#!/usr/bin/env python3
"""
Enhanced ICL Evaluation - FAST VERSION (~15 hours)
- 3 Prompt Templates
- Random + Embedding Top-k Selection  
- 3 runs with mean±std
- Shots: 0, 5, 10
- Reduced samples: 150 CLF, 100 NER
"""
...
# =============================================================================
# CONFIGURATION - REDUCED FOR ~15 HOURS
# =============================================================================
...
# REDUCED SETTINGS
SHOT_CONFIGS = [0, 5, 10]  # Removed 8-shot
NUM_RUNS = 3  # Reduced from 5
SEED_BASE = 42
MAX_NEW_TOKENS = 64
CLF_TEST_SUBSET = 150  # Reduced from 500
NER_TEST_SUBSET = 100  # Reduced from 300
...
def run_classification():
    ...
    log(f"Config: {len(SHOT_CONFIGS)} shots × 3 templates × 2 selections × {NUM_RUNS} runs × {CLF_TEST_SUBSET} samples")
    log("=" * 60)
    ...
    all_results = {}
    total_configs = len(LLM_MODELS) * len(CLF_TEMPLATES) * len(SHOT_CONFIGS) * 2
    config_count = 0
            ...
            for tmpl_name, tmpl in CLF_TEMPLATES.items():
                for k_shots in SHOT_CONFIGS:
                    for sel_name, selector in [('random', random_selector), ('topk', topk_selector)]:
                        ...
                        log(f"[{config_count}/{total_configs}] {config}")
                        ...
                        for run in range(NUM_RUNS):
                            ...
                            for _, row in tqdm(test_df.iterrows(), total=len(test_df), 
                                              desc=f"Run {run+1}/{NUM_RUNS}", leave=False):
                                text = row['clean_text']
                        ...
                        log(f"  Result: {mean_acc*100:.2f}% ± {std_acc*100:.2f}%")
...
def run_ner():
    log("=" * 60)
    log("NER EXPERIMENTS")
    log(f"Config: {len(SHOT_CONFIGS)} shots × 3 templates × 2 selections × {NUM_RUNS} runs × {NER_TEST_SUBSET} samples")
    log("=" * 60)
    ...
    all_results = {}
    total_configs = len(LLM_MODELS) * len(NER_TEMPLATES) * len(SHOT_CONFIGS) * 2
    config_count = 0
                ...
                for k_shots in SHOT_CONFIGS:
                    for sel_name, selector in [('random', random_selector), ('topk', topk_selector)]:
                        ...
                        for run in range(NUM_RUNS):
                            ...
                            for sample in tqdm(test_subset, desc=f"Run {run+1}/{NUM_RUNS}", leave=False):
                        ...
                        log(f"  Result: F1={mean_f1*100:.2f}% ± {std_f1*100:.2f}%")
...
if __name__ == "__main__":
    log("=" * 60)
    log("ENHANCED ICL EVALUATION - FAST VERSION")
    log(f"Started: {datetime.now()}")
    log(f"Expected time: ~15 hours")
    log("=" * 60)
